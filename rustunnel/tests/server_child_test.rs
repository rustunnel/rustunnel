...
fn test_valid() {
    let mut test_child = TestChild::new("rustunneld_test_ca").expect(error_line!());
    let client_tcp_stream = test_child.start_child("rustunneld_test_server").expect(error_line!());
    ...
    let mut client_tls_stream = client_tls_connector
        .connect("rustunneld_test_server", client_tcp_stream)
        .expect(error_line!());
...
fn test_wrong_ca_client_certificate() {
    let mut test_child = TestChild::new("rustunneld_test_ca").expect(error_line!());
    let client_tcp_stream = test_child.start_child("rustunneld_test_server").expect(error_line!());
    let mut test_wrong_ca = TestCa::generate("rustunneld_test_wrong_ca").expect(error_line!());
...
fn test_wrong_ca_client_certificate_same_issuer() {
    let mut test_child = TestChild::new("rustunneld_test_ca").expect(error_line!());
    let client_tcp_stream = test_child.start_child("rustunneld_test_server").expect(error_line!());
    let mut test_ca_2 = TestCa::generate("rustunneld_test_ca").expect(error_line!());
...
fn build_client(test_ca: &mut TestCa, valid_chain: bool) -> Result<TlsConnector, failure::Error> {
    let client_cert = test_ca
        .generate_signed_certificate("rustunneld_test_client", false)
        .expect(error_line!());
...
fn generate_self_signed_certificate_pkcs12() -> Result<Vec<u8>, failure::Error> {
    let test_ca = TestCa::generate("rustunneld_test_ca").expect(error_line!());
