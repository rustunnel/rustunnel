...
pub fn proxy_pipe_pair() -> Result<(ProxyPipeStream, TestPipeStream), failure::Error> {
    ...
    fn pipe2(flags: OFlag) -> nix::Result<(RawFd, RawFd)> {
        let (rx, tx) = unistd::pipe()?;
        fcntl::fcntl(rx, F_SETFL(flags))
            .and_then(|_| fcntl::fcntl(tx, F_SETFL(flags)))
            .map(|_| (rx, tx))
            .map_err(|error| {
                let _ignore = unistd::close(rx);
                let _ignore = unistd::close(tx);
                error
            })
    }

    let (source_pipe_0_rx, source_pipe_0_tx) = pipe2(OFlag::O_CLOEXEC | OFlag::O_NONBLOCK).context(error_line!())?;
    let (source_pipe_1_rx, source_pipe_1_tx) = pipe2(OFlag::O_CLOEXEC | OFlag::O_NONBLOCK).context(error_line!())?;
...
impl TestCa {
    ...
    pub fn to_pkcs12(&self) -> Result<Pkcs12, failure::Error> {
        ...
        pkcs12.ca(ca);
        let pkcs12 = pkcs12
            .build("", "rustunneld_test_pkcs12", &self.private_key, &self.certificate)
            .context(error_line!())?;
        Ok(pkcs12)
    }
    ...
    pub fn generate_server_identity(&mut self, subject_name_str: &str) -> Result<rustunnel::tls::Identity, failure::Error> {
        ...
        let tls_identity = rustunnel::tls::Identity::from_pkcs12(&server_certificate_pkcs12_der, "").context(error_line!())?;
        Ok(tls_identity)
    }
