...
use rustunnel::stream::ProxyPipeStream;
use rustunnel::tls::{Identity, TlsHostname};
use rustunnel::{ClientChild, ServerChild};
use rustunneld::config::*;
...
fn run_client(arguments: &clap::ArgMatches<'static>, log_level: log::Level) -> Result<(), failure::Error> {
    ...
    child::sandbox::close_all_fds(&close_fds).context("error closing all fds")?;
...
fn run_child(arguments: &clap::ArgMatches<'static>, log_level: log::Level) -> Result<(), failure::Error> {
    ...
    let source_tcp_stream = unsafe { TcpStream::from_raw_fd(source_fd) };
